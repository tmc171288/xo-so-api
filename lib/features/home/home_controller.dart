...
class HomeController extends GetxController {
  ...
  Future<void> _loadLiveResults() async {
    ...
    try {
      // 1. Fetch latest data from API first
      final result = await _resultRepository.getLatestResult(selectedRegion.value);
      if (result != null) {
        liveResults.add(result);
      }

      // 2. Then subscribe to socket for updates
  ...
  void _updateResult(LotteryResult result) {
    ...
    final index = liveResults.indexWhere((r) => r.region == result.region && r.date.day == result.date.day);
    if (index >= 0) {
